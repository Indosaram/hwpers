...
use hwpers::model::{
    header_footer::PageNumberFormat, hyperlink::Hyperlink, page_layout::PageOrientation,
};
use hwpers::writer::{style::ParagraphAlignment, HwpWriter};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    writer.add_aligned_paragraph("한글 문서 작성기 기능 시연", ParagraphAlignment::Center)?;

    writer.add_aligned_paragraph("Complete Feature Demonstration", ParagraphAlignment::Center)?;

    writer.add_paragraph("")?; // Empty line

    // 4. Section with hyperlinks
    println!("Adding hyperlinks section...");
    writer.add_aligned_paragraph("유용한 링크들", ParagraphAlignment::Left)?;
    ...
    writer.add_aligned_paragraph("왼쪽 정렬된 텍스트입니다.", ParagraphAlignment::Left)?;

    writer.add_aligned_paragraph("가운데 정렬된 텍스트입니다.", ParagraphAlignment::Center)?;

    writer.add_aligned_paragraph("오른쪽 정렬된 텍스트입니다.", ParagraphAlignment::Right)?;
    ...
    writer.add_aligned_paragraph("문서 내부 링크", ParagraphAlignment::Left)?;
    ...
    writer.add_aligned_paragraph("복합 스타일 예제", ParagraphAlignment::Center)?;
    ...
    writer.add_aligned_paragraph("문서 정보", ParagraphAlignment::Left)?;
...
fn create_hyperlink_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    writer.add_paragraph_with_hyperlinks("웹 링크: Rust 공식 웹사이트", vec![url_link])?;

    writer.add_paragraph_with_hyperlinks("이메일 링크: 이메일 보내기", vec![email_link])?;

    writer.add_paragraph_with_hyperlinks("파일 링크: 문서 열기", vec![file_link])?;
