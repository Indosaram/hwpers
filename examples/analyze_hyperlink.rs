...
fn analyze_document(file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(body_text) = document.body_texts.first() {
        println!("  - 첫 번째 섹션의 문단 수: {}", body_text.sections[0].paragraphs.len());
        ...
        for (i, paragraph) in body_text.sections[0].paragraphs.iter().enumerate() {
            if !paragraph.hyperlinks.is_empty() {
                ...
                for (j, hyperlink) in paragraph.hyperlinks.iter().enumerate() {
                    ...
                    println!("    시작 위치: {}, 길이: {}", hyperlink.start_position, hyperlink.length);
                    
                    // Count by type
                    *hyperlink_types.entry(format!("{:?}", hyperlink.hyperlink_type)).or_insert(0) += 1;
...
fn analyze_raw_structure(file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if hyperlink_positions.is_empty() {
        ...
        println!("    ✅ {} 개의 하이퍼링크 컨트롤 ID 발견:", hyperlink_positions.len());
        for (i, pos) in hyperlink_positions.iter().enumerate() {
            println!("      #{}: 파일 오프셋 0x{:08X} ({} bytes)", i + 1, pos, pos);
    ...
    if tag_54_positions.is_empty() {
        ...
        println!("    ✅ {} 개의 가능한 0x54 레코드 발견:", tag_54_positions.len());
        for (i, pos) in tag_54_positions.iter().take(10).enumerate() { // Show first 10 only
            ...
            if pos + 8 < full_buffer.len() {
                ...
                let size = u32::from_le_bytes([header_bytes[4], header_bytes[5], header_bytes[6], header_bytes[7]]);
                println!("        태그: 0x{:04X}, 레벨: {}, 크기: {} bytes", tag, level, size);
            }
        }
    }
    ...
    for term in search_terms {
        let utf16_bytes = term.encode_utf16()
