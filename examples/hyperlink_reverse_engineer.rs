        ...
        if data.len() > 28 {
            ...
            for start_offset in (28..data.len()).step_by(2) {
                if start_offset + 4 < data.len() {
                    ...
                    let potential_len =
                        u16::from_le_bytes([data[start_offset], data[start_offset + 1]]) as usize;

                    // Reasonable string length (1-50 characters)
                    if potential_len > 0
                        && potential_len <= 50
                        && start_offset + 2 + potential_len * 2 <= data.len()
                    {
                        let mut utf16_chars = Vec::new();
                        let mut valid = true;

                        for char_offset in
                            (start_offset + 2..start_offset + 2 + potential_len * 2).step_by(2)
                        {
                        ...
                        if valid && !utf16_chars.is_empty() {
                            if let Ok(text) = String::from_utf16(&utf16_chars) {
                                ...
                                if !clean_text.is_empty()
                                    && (clean_text
                                        .chars()
                                        .any(|c| c.is_alphabetic() || is_hangul(c)))
                                {
                                    found_strings.push((
                                        start_offset,
                                        potential_len,
