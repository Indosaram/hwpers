...
use crate::render::layout::{LayoutEngine, RenderedPage, RenderedParagraph, RenderedLine, TextRun};
...
impl<'a> HwpRenderer<'a> {
    ...
    fn render_line(&self, line: &RenderedLine, _para_shape: Option<&ParaShape>) -> Vec<RenderElement> {
        let mut elements = Vec::new();
    ...
    fn render_text_run(&self, run: &TextRun, baseline_y: i32) -> Option<RenderElement> {
        ...
        let font_face = self.document.get_face_name(char_shape.face_name_ids[0] as usize)?;
...
impl RenderResult {
    ...
    pub fn to_svg(&self, page_index: usize) -> Option<String> {
        ...
        for element in &page.elements {
            match element {
                RenderElement::Text { x, y, text, font_family, font_size, color, bold, italic, underline } => {
                    svg.push_str(&format!(
                ...
                RenderElement::Line { x1, y1, x2, y2, color, width } => {
                    svg.push_str(&format!(
                        "<line x1=\"{}\" y1=\"{}\" x2=\"{}\" y2=\"{}\" stroke=\"#{:06X}\" stroke-width=\"{}\"/>",
                        x1, y1, x2, y2, color & 0xFFFFFF, width
                    ));
                }
                RenderElement::Rectangle { x, y, width, height, fill_color, stroke_color, stroke_width } => {
                    svg.push_str(&format!(r#"<rect x="{}" y="{}" width="{}" height="{}""#, x, y, width, height));
                    ...
                    if let Some(stroke) = stroke_color {
                        svg.push_str(&format!(" stroke=\"#{:06X}\" stroke-width=\"{}\"", stroke & 0xFFFFFF, stroke_width));
                    }
