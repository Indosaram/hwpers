...
fn test_simple_table() {
    ...
    let cell_texts: Vec<&str> = document.body_texts[0].sections[0].paragraphs
        .iter()
...
fn test_table_with_long_text() {
    ...
    assert!(long_text_para.text.as_ref().unwrap().content.contains("This is a very long text"));
}
...
fn test_mixed_content_with_table() {
    ...
    // Check content order
    assert!(paragraphs[0].text.as_ref().unwrap().content.contains("Before table"));
    assert!(paragraphs[1].ctrl_header.is_some()); // Table control
    assert!(paragraphs[6].text.as_ref().unwrap().content.contains("After table"));
}