...
fn test_bold_ranges() {
    ...
    assert!(paragraph.text.is_some());
    assert_eq!(paragraph.text.as_ref().unwrap().content, "This is bold text");
    assert!(paragraph.char_shapes.is_some());
...
fn test_highlight_ranges() {
    ...
    assert!(paragraph.text.is_some());
    assert_eq!(paragraph.text.as_ref().unwrap().content, "Text with yellow highlight");
    assert!(paragraph.char_shapes.is_some());
...
fn test_color_ranges() {
    ...
    assert!(paragraph.text.is_some());
    assert_eq!(paragraph.text.as_ref().unwrap().content, "Red and blue text");
    assert!(paragraph.char_shapes.is_some());
    assert_eq!(paragraph.char_shape_count, 2); // Two color ranges
}
