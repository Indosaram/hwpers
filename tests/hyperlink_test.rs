...
fn test_basic_hyperlink() {
    ...
    // Add content
    writer
        .add_paragraph("This document contains hyperlinks.")
        .unwrap();

    // Add basic hyperlink
    writer
        .add_hyperlink("Visit our website", "https://example.com")
        .unwrap();
...
fn test_email_hyperlink() {
    ...
    writer
        .add_email_link("Send us an email", "contact@example.com")
        .unwrap();

    let document = writer.document();
    let section = &document.body_texts[0].sections[0];
    let hyperlink_para = section
        .paragraphs
        .iter()
        .find(|p| !p.hyperlinks.is_empty())
        .unwrap();
    let hyperlink = &hyperlink_para.hyperlinks[0];
...
fn test_file_hyperlink() {
    ...
    writer
        .add_file_link("Download manual", "/documents/manual.pdf")
        .unwrap();
...
fn test_bookmark_hyperlink() {
    ...
    writer
        .add_bookmark_link("Go to Chapter 1", "chapter1")
        .unwrap();
...
fn test_multiple_hyperlinks_in_paragraph() {
    ...
    writer
        .add_paragraph_with_hyperlinks(text, hyperlinks)
        .unwrap();
...
fn test_hyperlink_serialization() {
    ...
    assert_eq!(
        parsed_hyperlink.open_in_new_window,
        hyperlink.open_in_new_window
    );
...
fn test_mixed_document_with_hyperlinks() {
    ...
    writer
        .add_paragraph("This document demonstrates hyperlink functionality.")
        .unwrap();

    // Add different types of hyperlinks
    writer
        .add_hyperlink("Visit our homepage", "https://example.com")
        .unwrap();
    writer
        .add_email_link("Contact support", "support@example.com")
        .unwrap();
    writer
        .add_file_link("Download PDF", "/docs/guide.pdf")
        .unwrap();
    writer
        .add_bookmark_link("Go to conclusion", "conclusion")
        .unwrap();
    ...
    // Count hyperlinks
    let total_hyperlinks: usize = section.paragraphs.iter().map(|p| p.hyperlinks.len()).sum();
    assert_eq!(total_hyperlinks, 5); // 5 hyperlink paragraphs

    // Check different hyperlink types
    let hyperlink_types: Vec<HyperlinkType> = section
        .paragraphs
        .iter()
