use hwpers::{HwpReader, HwpWriter, writer::style::TextStyle};
...
fn test_add_paragraph_with_style() {
    ...
    writer.add_paragraph_with_style("Bold text", &bold_style).unwrap();
...
fn test_font_management() {
    ...
    writer.add_paragraph_with_style("굴림체 텍스트", &gulim_style).unwrap();
    
    let batang_style = TextStyle::new().font("바탕");
    writer.add_paragraph_with_style("바탕체 텍스트", &batang_style).unwrap();
    
    // Use the same font again
    writer.add_paragraph_with_style("또 다른 굴림체 텍스트", &gulim_style).unwrap();
    ...
    // Verify font names
    let font_names: Vec<&str> = document.doc_info.face_names.iter()
...
fn test_styled_document_roundtrip() {
    ...
    writer.add_paragraph_with_style("Bold 14pt text", &bold_style).unwrap();
    
    let colored_style = TextStyle::new().color(0x0000FF).italic();
    writer.add_paragraph_with_style("Blue italic text", &colored_style).unwrap();
