...
fn test_add_image_from_bytes() {
    ...
    writer.add_image_from_bytes(&test_png, ImageFormat::Png).unwrap();
...
fn test_image_with_options() {
    ...
    writer.add_image_with_options(&test_png, ImageFormat::Png, &options).unwrap();
...
fn test_mixed_content_with_images() {
    ...
    assert!(paragraphs[0].text.as_ref().unwrap().content.contains("Images in Document"));
    assert!(paragraphs[1].text.as_ref().unwrap().content.contains("Here is an image"));
    assert!(paragraphs[2].ctrl_header.is_some()); // Picture control
    assert!(paragraphs[3].text.as_ref().unwrap().content.contains("그림: Sample Image"));
    assert!(paragraphs[4].text.as_ref().unwrap().content.contains("Text after the image"));
}