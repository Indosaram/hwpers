use hwpers::{HwpWriter, model::{PageOrientation, PaperSize, PageMargins, PageLayout, mm_to_hwp_units, inches_to_hwp_units}};
...
fn test_page_margins_mm() {
    ...
    writer.add_paragraph("Testing margins in millimeters").unwrap();
...
fn test_page_background_color() {
    ...
    writer.add_paragraph("Testing page background color").unwrap();
...
fn test_margin_builders() {
    ...
    let margins_with_hf = PageMargins::new_mm(20.0, 20.0, 20.0, 20.0)
        .with_header_footer_mm(12.0, 10.0);
    assert_eq!(margins_with_hf.header, mm_to_hwp_units(12.0));
    assert_eq!(margins_with_hf.footer, mm_to_hwp_units(10.0));
    
    // Test with_gutter_mm
    let margins_with_gutter = PageMargins::new_mm(20.0, 20.0, 20.0, 20.0)
        .with_gutter_mm(5.0);
    assert_eq!(margins_with_gutter.gutter, mm_to_hwp_units(5.0));
    
    // Test with_mirror_margins
    let margins_mirror = PageMargins::new_mm(20.0, 20.0, 20.0, 20.0)
        .with_mirror_margins(true);
...
fn test_complex_page_layout() {
    ...
    writer.add_paragraph("This document demonstrates complex page layout features.").unwrap();
...
fn test_mixed_document_with_page_layout() {
    ...
    writer.add_heading("Document with Custom Page Layout", 1).unwrap();
    writer.add_paragraph("This document uses Letter paper size with normal margins.").unwrap();
    ...
    // Add hyperlinks
    writer.add_hyperlink("Letter paper info", "https://en.wikipedia.org/wiki/Letter_(paper_size)").unwrap();
    
    // Add text box
    writer.add_styled_text_box("Note: This document uses Letter paper size which is common in North America.", "info").unwrap();
