use hwpers::{HwpWriter, model::{TextBoxAlignment, TextBoxBorderStyle, TextBoxFillType}};
...
fn test_basic_text_box() {
    ...
    // Add content
    writer.add_paragraph("This document contains a text box.").unwrap();
...
fn test_positioned_text_box() {
    ...
    writer.add_text_box_at_position("Positioned text box", 10, 20, 50, 30).unwrap();
    
    let document = writer.document();
    let section = &document.body_texts[0].sections[0];
    let text_box_para = section.paragraphs.iter().find(|p| p.text_box_data.is_some()).unwrap();
    let text_box = text_box_para.text_box_data.as_ref().unwrap();
...
fn test_styled_text_boxes() {
    ...
    writer.add_paragraph("Testing different text box styles").unwrap();
    
    // Test different predefined styles
    writer.add_styled_text_box("Basic style", "basic").unwrap();
    writer.add_styled_text_box("Highlight style", "highlight").unwrap();
    writer.add_styled_text_box("Warning style", "warning").unwrap();
    writer.add_styled_text_box("Info style", "info").unwrap();
    writer.add_styled_text_box("Transparent style", "transparent").unwrap();
    writer.add_styled_text_box("Bubble style", "bubble").unwrap();
    
    let document = writer.document();
    let section = &document.body_texts[0].sections[0];
    
    // Count text box paragraphs
    let text_box_count = section.paragraphs.iter()
        .filter(|p| p.text_box_data.is_some())
        .count();
    assert_eq!(text_box_count, 6);
    
    // Test specific styles
    let highlight_para = section.paragraphs.iter()
        .find(|p| p.text_box_data.as_ref().map_or(false, |tb| tb.text == "Highlight style"))
        .unwrap();
    ...
    let warning_para = section.paragraphs.iter()
        .find(|p| p.text_box_data.as_ref().map_or(false, |tb| tb.text == "Warning style"))
        .unwrap();
...
fn test_multiple_text_boxes() {
    ...
    writer.add_paragraph("Document with multiple text boxes").unwrap();
    
    // Add various text boxes
    writer.add_text_box("First text box").unwrap();
    writer.add_text_box_at_position("Second text box", 50, 60, 40, 25).unwrap();
    writer.add_styled_text_box("Third text box", "info").unwrap();
    writer.add_custom_text_box(
        "Fourth text box",
        100, 120,
        70, 35,
    ...
    let text_boxes: Vec<_> = section.paragraphs.iter()
...
fn test_mixed_document_with_text_boxes() {
    ...
    writer.add_heading("Document with Mixed Content", 1).unwrap();
    writer.add_paragraph("This document demonstrates text boxes alongside other content.").unwrap();
    ...
    // Add text boxes
    writer.add_styled_text_box("Highlighted note", "highlight").unwrap();
    writer.add_text_box_at_position("Side comment", 120, 80, 60, 25).unwrap();
    
    // Add more content
    writer.add_paragraph("Additional content after text boxes.").unwrap();
    ...
    let text_box_count = section.paragraphs.iter()
        .filter(|p| p.text_box_data.is_some())
        .count();
    let table_count = section.paragraphs.iter()
        .filter(|p| p.table_data.is_some())
        .count();
    let text_paragraph_count = section.paragraphs.iter()
